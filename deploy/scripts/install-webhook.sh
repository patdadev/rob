...
log() {
  printf '[install-webhook] %s\n' "$*"
}

warn() {
  printf '[install-webhook] WARNING: %s\n' "$*" >&2
}
...
APP_ENV=prod
LOG_LEVEL=INFO
DATABASE_URL=postgresql://prod_rob_webhook:replace@replace:25060/rob_prod?sslmode=require
...
# Notify the bot through its private ops bridge as soon as a send is recorded.
...
warn_if_stale_env_values() {
  ...
  if grep -Eq 'dev_rob_bot|dev_rob_webhook|rob-dev\.barecoding\.com|rob_dev_v2' "${env_file}"; then
    warn "Existing .env appears to contain old rehearsal values."
    warn "This installer will not overwrite it."
    warn "Please update DATABASE_URL to prod_rob_webhook on rob_prod and confirm ROB_BOT_NOTIFY_URL points at the bot's private ops bridge."
  fi
}
...
Webhook production bootstrap complete.
