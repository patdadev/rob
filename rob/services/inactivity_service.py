...
class InactivityService:
    ...
    def _format_remove_time(self, moment: datetime) -> str:
    ...
    def _build_first_notice(self, member: discord.Member, remove_at: datetime) -> str:
        return (
            "## Inactivity System Notice\n\n"
            f"Hey {member.mention}, you're currently marked as inactive.\n"
            f"If this does not change, removal is scheduled for {self._format_remove_time(remove_at)}.\n\n"
            f"To stay active, say hi in {self._notice_channel_hint()}."
        )

    def _build_warning_notice(self, member: discord.Member, remove_at: datetime) -> str:
        display_name = (member.nick or member.display_name or member.name).strip() or member.name
        return (
            "## Inactivity Warning\n\n"
            f"Hey {display_name}, this is a warning that your inactivity removal is scheduled for "
            f"{self._format_remove_time(remove_at)}.\n\n"
            f"If you want to remain in the server, please become active in {self._notice_channel_hint()}."
        )

    def _build_final_notice(self, member: discord.Member, remove_at: datetime) -> str:
        return (
            "## Final Inactivity Notice\n\n"
            f"Hey {member.mention}, this is your final warning before removal.\n"
            f"Current removal schedule: {self._format_remove_time(remove_at)}.\n\n"
            f"If you want to stay, become active in {self._notice_channel_hint()} now."
        )

    async def _send_dm(self, member: discord.Member, *, body: str, label: str) -> None:
        try:
    ...
    async def _save_member_state(
        self,
        guild_id: int,
        member_id: int,
        *,
        assigned_at: datetime | None | object = _STATE_UNSET,
        remove_at: datetime | None | object = _STATE_UNSET,
        initial_notice_sent: bool | None | object = _STATE_UNSET,
        final_notice_sent: bool | None | object = _STATE_UNSET,
    ) -> None:
        keys = self._member_keys(guild_id, member_id)
        ...
        if initial_notice_sent is not _STATE_UNSET:
            values[keys["initial_notice_sent"]] = (
                "true" if bool(initial_notice_sent) else "false"
            ) if isinstance(initial_notice_sent, bool) else None
        if final_notice_sent is not _STATE_UNSET:
            values[keys["final_notice_sent"]] = (
                "true" if bool(final_notice_sent) else "false"
            ) if isinstance(final_notice_sent, bool) else None
    ...
    async def clear_member_state(self, guild_id: int, member_id: int) -> None:
        await self._save_member_state(
            ...
            assigned_at=None,
            remove_at=None,
            initial_notice_sent=None,
            final_notice_sent=None,
        )
    ...
    async def process_guild(
        ...
        *,
        ...
        for member in members:
            ...
            initial_notice_is_bootstrap = False
            ...
            if assigned_at is None or remove_at is None:
                ...
                initial_notice_is_bootstrap = is_bootstrap_run
                await self._save_member_state(
                    guild_id,
                    member.id,
            ...
            if send_notifications and not initial_notice_sent:
                if initial_notice_is_bootstrap or (remove_at - assigned_at) >= self.bootstrap_grace:
                    body = self._build_first_notice(member, remove_at)
                    label = "bootstrap-notice"
                else:
                    body = self._build_warning_notice(member, remove_at)
                    label = "warning-notice"
                await self._send_dm(member, body=body, label=label)
            ...
            if (
                send_notifications
                and not final_notice_sent
                and now < remove_at
                and (remove_at - now) <= self.final_notice_window
            ):
                await self._send_dm(
                    member,
                    body=self._build_final_notice(member, remove_at),
                    label="final-notice",
                )
